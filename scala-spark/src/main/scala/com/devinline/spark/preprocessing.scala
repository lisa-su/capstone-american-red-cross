...
object preprocessing {
  ...
  def main(args: Array[String]) = {
    ...
    val data = sc.textFile("data/state13.csv")
    ...
    //TODO: Remove the comments for file saving
    val repeat_donor = donate_cnt.filter(v => v._2 > 1 ).map(v => v._1)//.saveAsTextFile("data/temp1")
    val onetime_donor = donate_cnt.filter(v => v._2 == 1 ).map(v => v._1)//.saveAsTextFile("data/temp2")
    
    val repeat_donor_sample = repeat_donor.sample(false, 0.01, 100)
    val onetime_donor_sample = onetime_donor.sample(false, 0.01, 100)
    val sample_donors = repeat_donor_sample.collect() ++ onetime_donor_sample.collect()
    var sample_data = data_after_2000.filter { row => sample_donors contains row(0) }
    
    sample_data.saveAsTextFile("data/sample_data")
